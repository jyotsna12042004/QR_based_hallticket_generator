 ...
 * @author NARESH
 */
public class HTApprove extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            try {
                int i = sto.executeUpdate("update hall_ticket_requests set htstatus='Approved' where id='" + reqid + "'");
                ...
                if (i != 0) {
                    ...
                    String updateQuery = "UPDATE hall_ticket_requests SET image_data = ?,unique_id = ? WHERE id = ?";
                    ...
                    response.sendRedirect("HallTicketReq.jsp?Approved");
                } else {
                    System.out.println("failed");
                    response.sendRedirect("HallTicketReq.jsp?Failed");
                }
            } catch (IOException | SQLException ex) {
            } catch (WriterException ex) {
                Logger.getLogger(HTApprove.class.getName()).log(Level.SEVERE, null, ex);
            }
